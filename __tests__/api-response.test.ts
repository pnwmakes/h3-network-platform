...
describe('API Response Utilities', () => {
    ...
    describe('Cache Headers', () => {
        ...
        it('should have different max-age values', () => {
            expect(CacheHeaders.shortCache['Cache-Control']).toContain(
                'max-age=300'
            );
            expect(CacheHeaders.mediumCache['Cache-Control']).toContain(
                'max-age=1800'
            );
            expect(CacheHeaders.longCache['Cache-Control']).toContain(
                'max-age=86400'
            );
            expect(CacheHeaders.staticAssets['Cache-Control']).toContain(
                'max-age=31536000'
            );
        });
    });
