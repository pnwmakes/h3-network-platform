    ...
    useEffect(() => {
        if (!session) {
            ...
            if (!document.cookie.includes('guest-session')) {
                ...
                const guestSessionId = `guest-${Date.now()}-${Math.random()
                    .toString(36)
                    .substr(2, 9)}`;
                document.cookie = `guest-session=${guestSessionId}; path=/; max-age=${
                    60 * 60 * 24 * 30
                }`; // 30 days
            }
        }
        ...
        <div className={cn('flex items-center gap-1', className)}>
            ...
            >
                ...
                {likeData.isLiked && !isLoading && !isPending && (
                    <Heart className='absolute inset-0 m-auto h-5 w-5 text-red-500 fill-current animate-ping opacity-25' />
                )}
            </Button>
