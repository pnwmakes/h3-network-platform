...
export async function GET(request: NextRequest) {
  try {
    ...
    videosByTopic.forEach(item => {
      ...
      existing.avgViews = (existing.avgViews + (item._avg.viewCount || 0)) / 2
      existing.avgLikes = (existing.avgLikes + (item._avg.likeCount || 0)) / 2
    })
    ...
    const contentByCategory = Array.from(topicMap.entries()).map(([topic, data]) => ({
      category: topic,
      count: data.count,
    ...
    const totalViews = (videoStats._sum.viewCount || 0) + (blogStats._sum.viewCount || 0)
    const totalLikes = (videoStats._sum.likeCount || 0) + (blogStats._sum.likeCount || 0)

    const avgEngagementRate = totalLikes && totalViews 
    ...
    const goals = [
      {
        ...
        target: 10000,
        progress: Math.min((activeUsersToday * 30 / 10000) * 100, 100)
      },
      {
