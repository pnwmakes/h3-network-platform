...
export default async function CreatorPage({ params }: CreatorPageProps) {
    ...
    const totalViews = creator.videos.reduce((sum, video) => sum + video.viewCount, 0) +
                     creator.blogs.reduce((sum, blog) => sum + blog.viewCount, 0);
    const totalWatchTime = creator.videos.reduce((sum, video) => sum + (video.duration || 0), 0);
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <div className="relative h-48 bg-gradient-to-br from-blue-100 via-purple-100 to-green-100">
                        ...
                        <div className="absolute bottom-6 left-6 flex items-end space-x-6">
                            ...
                            <div className="text-white mb-4">
                                ...
                                {creator.showName && (
                                    <p className="text-lg opacity-90 mb-2">{creator.showName}</p>
                                )}
                    ...
                    <div className="p-6">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                            ...
                            <div className="text-center p-4 bg-yellow-50 rounded-lg">
                                ...
                                <div className="text-2xl font-bold text-gray-900">{Math.floor(totalWatchTime / 60)}</div>
                ...
                <div className="bg-white rounded-xl shadow-lg">
                    <div className="border-b border-gray-200">
                        <nav className="flex space-x-8 px-6" aria-label="Tabs">
                            <a href="#videos" className="border-b-2 border-blue-500 py-4 px-1 text-sm font-medium text-blue-600">
                                Videos ({totalVideos})
                            </a>
                            {totalBlogs > 0 && (
                                <a href="#blogs" className="border-b-2 border-transparent py-4 px-1 text-sm font-medium text-gray-500 hover:text-gray-700 hover:border-gray-300">
                                    Blogs ({totalBlogs})
                                </a>
                            )}
                        </nav>
                        ...
                        {creator.videos.length > 0 ? (
                                            ...
                                            {video.topic && (
                                                <div className="mt-2">
                                                    <span className="inline-block bg-gray-200 text-gray-700 text-xs px-2 py-1 rounded-full">
                                                        {video.topic.replace('_', ' ').toLowerCase()}
                                                    </span>
                                                </div>
                                            )}
                                        </div>
                            ...
                            <div className="text-center py-12">
                                ...
                                <p className="text-gray-500">
                                    {creator.displayName} hasn&apos;t published any videos yet.
                                </p>
                            </div>
                        )}
                    </div>
                    ...
                    {totalBlogs > 0 && (
                        <div className="p-6 border-t border-gray-200" id="blogs">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {creator.blogs.map((blog) => (
                                    ...
                                    >
                                        ...
                                        {blog.topic && (
                                            <div className="mt-2">
                                                <span className="inline-block bg-gray-200 text-gray-700 text-xs px-2 py-1 rounded-full">
                                                    {blog.topic.replace('_', ' ').toLowerCase()}
                                                </span>
                                            </div>
                                        )}
                                    </Link>
                    ...
                    {totalBlogs === 0 && (
                        <div className="p-6 border-t border-gray-200 hidden" id="blogs">
                            <div className="text-center py-12">
                                <FileText className="h-16 w-16 text-gray-300 mx-auto mb-4" />
                                <h3 className="text-lg font-medium text-gray-900 mb-2">No blogs yet</h3>
                                <p className="text-gray-500">
                                    {creator.displayName} hasn&apos;t published any blogs yet.
                                </p>
                            </div>
                        </div>
                    )}
                ...
                <div className="mt-8 text-center bg-white rounded-xl shadow-lg p-8">
                    ...
                    <p className="text-gray-600 mb-6">
                        Stay updated with the latest content from {creator.displayName} and support their mission of Hope, Help, and Humor.
                    </p>
                    <div className="space-y-3">
                        ...
                        <div className="text-sm text-gray-500">
                            Get notified when {creator.displayName} posts new content
                        </div>
                    </div>
                </div>
            </div>
