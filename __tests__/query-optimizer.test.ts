...
describe('Query Optimizer', () => {
    ...
    describe('Order By Clauses', () => {
        it('should have video ordering options', () => {
            expect(QueryOptimizer.ORDER_BY.videos.recent).toEqual({
                publishedAt: 'desc',
            });
            expect(QueryOptimizer.ORDER_BY.videos.popular).toEqual({
                viewCount: 'desc',
            });
            expect(QueryOptimizer.ORDER_BY.videos.trending).toBeInstanceOf(
                Array
            );
        });

        it('should have creator ordering options', () => {
            expect(QueryOptimizer.ORDER_BY.creators.popular).toEqual({
                followerCount: 'desc',
            });
            expect(QueryOptimizer.ORDER_BY.creators.recent).toEqual({
                createdAt: 'desc',
            });
            expect(QueryOptimizer.ORDER_BY.creators.alphabetical).toEqual({
                displayName: 'asc',
            });
        });
    });
