import { PrismaClient, UserRole, ContentStatus, ContentTopic } from '@prisma/client';
...
async function main() {
  ...
  const show1 = await prisma.show.upsert({
    ...
    create: {
      name: 'Second Chances',
      description: 'Stories of redemption, second chances, and transformation from formerly incarcerated individuals.',
      isActive: true,
    }
  });
  ...
  const show2 = await prisma.show.upsert({
    ...
    create: {
      name: 'Recovery Roads',
      description: 'Navigating addiction recovery and reentry challenges with expert guidance and real stories.',
      isActive: true,
    }
  });
  ...
  const videos = [
    {
      ...
      description: 'An introduction to the key issues in criminal justice reform and why it matters for communities.',
    ...
    },
    {
      ...
      description: 'Personal stories and practical advice for individuals reentering society after incarceration.',
    ...
    },
    {
      ...
      description: 'Understanding addiction as a disease and the path to recovery and healing.',
    ...
    },
    {
      ...
      description: 'How families can support their loved ones through addiction recovery and reentry.',
    ...
    },
    {
      ...
      description: 'Examining recent policy changes in criminal justice and their real-world impact.',
...
If you're struggling with reentry, addiction, or just feeling lost, know that you're not alone. We're here to support each other on this journey.`,
      excerpt: 'A personal story of transformation from incarceration to purpose, and the power of community support in the reentry process.',
      topic: ContentTopic.REENTRY,
      status: ContentStatus.PUBLISHED,
      creatorId: creator1.creator!.id,
...
Remember: Recovery is not linear, and every person's journey is different. What matters is that we keep supporting each other along the way.`,
      excerpt: 'Breaking down the misconceptions about addiction and explaining why treatment, not punishment, is the answer.',
      topic: ContentTopic.ADDICTION,
      status: ContentStatus.PUBLISHED,
      creatorId: creator2.creator!.id,
  ...
  for (const blogData of blogPosts) {
    await prisma.blog.create({
      data: {
        ...blogData,
        publishedAt: new Date(Date.now() - Math.random() * 15 * 24 * 60 * 60 * 1000), // Random date within last 15 days
      }
    });
  }
